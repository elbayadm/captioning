...
def get_words_matrix(model):
    # Load pytorch model and fetch word embedding:
    pth = torch.load('save/%s/model-best.pth' % model, map_location={'cuda:0': 'cpu'})
    W = pth['embed.weight']
    W = W.cpu().numpy()
    init = tf.constant(W)
    Wtf = tf.get_variable('words_%s' % model, initializer=init)
    ...
    embed.tensor_name = 'embedding:%s' % model
    ...
    saver.save(sess, os.path.join(FLAGS.log_dir, 'projector/plot.ckpt'),
               global_step=1)
...
def generate_embeddings():
    ...
    embed.metadata_path = os.path.join(FLAGS.log_dir + '/projector/metadata.tsv')
    embed.sprite.image_path = os.path.join(FLAGS.log_dir + '/mnist_10k_sprite.png')
...
def generate_words_metadata_file():
    # Import data
    infos = pickle.load(open('%s/infos.pkl' % FLAGS.log_dir))
    vocab = infos['vocab']
...
def main(_):
    ...
    #  generate_words_metadata_file()
    #  get_words_matrix(FLAGS.model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='./scripts',
                        help='Directory for storing input data')
    parser.add_argument('--log_dir', type=str, default='./save/tmptf',
                        help='Summaries log directory')

    parser.add_argument('--model', type=str, default='baseline_resnet50_nobos',
                        help='Summaries log directory')
    FLAGS, unparsed = parser.parse_known_args()
    FLAGS.log_dir = './save/%s' % FLAGS.model
